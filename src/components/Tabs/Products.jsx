...
const Products = ({ setActiveTab }) => {
  ...
  useEffect(() => {
    if (product && product.product_id && selectedCategory.code === "" && editingProduct === true) {
      setSelectedCategory({
        code: product.category_id,
        name: getCategoryName(product.category_id),
      });
    }
    if (product && product.product_id && selectedStatus.code === "" && editingProduct === true) {
      let status = statuses.find((status) => status.code === product.is_available);
      setSelectedStatus(status || { code: "", name: "" });
    }
    if (product && product.product_id && selectedCurrency.code === "" && editingProduct === true) {
  ...
  const validateProduct = (data) => {
    ...
    if ((!editingProduct) && (data.image1 === undefined || data.image1 === null)) {
  ...
  const handleProductSubmit = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      if (product.product_id) {
        ...
        dispatch(UpdateProduct(product, showError, showWarning, showSuccess));
      } else {
        setIsLoading(true);
  ...
  const handleViewProduct = (productInfo) => {
    ...
    const children = (
      <div className="admin-card" style={{ textAlign: "left" }}>
        <label className="form-label">Nombre: <span style={{ fontWeight: "bold" }}>{productInfo.name}</span></label>
        <label className="form-label">Descripción: <span style={{ fontWeight: "bold" }}>{productInfo.description}</span></label>
        <label className="form-label">Precio: <span style={{ fontWeight: "bold" }}> {productInfo.currency}{productInfo.price}</span></label>
        <label className="form-label">Categoría: <span style={{ fontWeight: "bold" }}>{getCategoryName(productInfo.category_id)}</span></label>
        <label className="form-label">Estado: <span className={`product-status  product-status_${available ? "available" : "unavailable"
          }`}> {available ? "Disponible" : "Agotado"}</span></label>
        <label className="form-label">Moneda: <span style={{ fontWeight: "bold" }}>{selectedCurrency.name}</span></label>
      </div>
    )
    const footer = null
    setDialogContent({ title, children, footer });
    ...
    return (
      <div className="p-3">
        <DataTable value={data.imagesUrl}>
          <Column header="Imagenes" body={(rowData) => {
              ...
              <div className="flex justify-content-center">
                <Button
                  ...
                  onClick={() => {
                    ...
                    const children = (
                      <div>¿Estás seguro de que deseas eliminar esta imagen?</div>
                    )
                    const footer = (
                      <div className="flex justify-content-end">
                        <Button
        ...
        <div className="admin-card">
          ...
          {products.length === 0 ? (
            ...
            <>
              ...
              >
                <p>{dialogContent?.children || "¿Estás seguro de que deseas eliminar este producto?"}</p>
              </DialogModal>
              {!isMobile && (
                <div className="card">
                  <DataTable value={products} expandedRows={expandedRows} onRowToggle={(e) => setExpandedRows(e.data)}
                    ...
                    <Column field="categoryId" header="Categoría" body={(rowData) => getCategoryName(rowData.category_id)}></Column>
                    <Column header="Estado" body={(rowData) => {
                      let available = rowData.is_available === "available" ? true : false;
                      return (
                        <span
                          className={`product-status  product-status_${available ? "available" : "unavailable"
                            }`}
                        >
              ...
              {isMobile && (
                <div className="card">
                  <DataTable value={products} expandedRows={expandedRows} onRowToggle={(e) => setExpandedRows(e.data)}
                    rowExpansionTemplate={rowExpansionTemplate}
                    dataKey="product_id" tableStyle={{ minWidth: '6rem' }}>
                    <Column header="Nombre" body={(rowData) => {
                      return (
                        <>
                          <Card >
                            <div >
